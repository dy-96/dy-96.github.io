...
let startAngles = {}; // Store starting angles for each touch
let animationFrame;
...
function rotateSteeringWheel(touchId, x, y) {
    const newAngle = Math.atan2(y, x);
    let deltaAngle = newAngle - startAngles[touchId];
...
steeringWheel.addEventListener("touchstart", (e) => {
    ...
    for (let touch of e.touches) {
        ...
        startAngles[touch.identifier] = Math.atan2(y - steeringWheel.offsetHeight / 2, x - steeringWheel.offsetWidth / 2); // Store the starting angle for the touch
    }
...
steeringWheel.addEventListener("touchmove", (e) => {
    if (isDragging) {
        for (let touch of e.touches) {
            const { x, y } = getAverageTouchPos(e.touches);
            rotateSteeringWheel(touch.identifier, x - steeringWheel.offsetWidth / 2, y - steeringWheel.offsetHeight / 2);
        }
...
steeringWheel.addEventListener("touchend", (e) => {
    ...
    for (let touch of e.changedTouches) {
        delete startAngles[touch.identifier]; // Remove the starting angle for this touch
    }
});
